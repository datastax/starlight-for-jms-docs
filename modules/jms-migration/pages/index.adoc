...
:description: Use the Starlight for JMS API to produce and consume messages on {pulsar-short} topics.

The {jms_repo}[Starlight for JMS] extension is a highly compliant JMS implementation designed to run on a modern streaming platform.

This guide explains how to prepare an {pulsar-reg} streaming instance, and then use Starlight for JMS to produce and consume messages with a Java JMS client.
...
+
--
* An active Astra Streaming {pulsar-short} cluster with a tenant, namespace, and topic that you want to use with Starlight for JMS.
...
== Produce and consume a message with Starlight for JMS
...
. Open the project in your IDE, and then add the Starlight for JMS dependency to the `pom.xml`:
+
.pom.xml
[source,xml]
----
