...
Server-side filtering requires {pulsar-short} version 2.10.1 or later, or {luna-streaming} version 2.10 or later.
...
. Install {product} with the fat JAR that includes all dependencies. Put the `.nar` file in `/pulsar/filters`.
+
[NOTE]
====
...
{product} allows you to pre-configure the `selector` property when creating your {pulsar-short} subscription. Modify `PulsarConnectionFactory` in {product} to include:
...
This only partially works for a non-JMS client, because batch messages cannot be completely filtered out by the Broker. If you use this feature, you should use the {product} 2.x client and the `jms.useServerSideFiltering` feature.
...
* *Batch messages* - Server-side filtering works on the Entry level in {pulsar-short}. An *Entry* is a *single message* or a *single batch of messages*. The Broker can't filter out a few messages in a batch; it either passes completely or fails completely.
+
...
* *Increased Broker resources usage* - The {pulsar-short} Broker does not perform processing on messages by default. When server-side filtering is enabled, the Broker will require additional CPU and heap memory to process filters.
+
...
* *Unsupported message types* - {product} does not support encrypted batch messages because the filter can't decode the payload of the entry.
For the same reason, {product} does not support custom formats like native Kafka messages.
