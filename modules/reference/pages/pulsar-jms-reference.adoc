...
See xref:pulsar-jms-implementation.adoc#transaction-emulations[Transaction Emulation].
...
| boolean
| false
a| Offloads message filtering to the Broker nodes. See xref:pulsar-jms-server-side-filters.adoc[Server Side Filters].
...
Please refer to xref:pulsar-jms-implementation.adoc[Implementation details] in order to read about how {starlight_jms} emulates some features that are not directly supported by Pulsar.
