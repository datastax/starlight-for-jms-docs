...
| long
| Disabled
| Set timeout for unacked messages.
...
| no
| String
| If not specified, for {pulsar-short} standalone instances running on localhost, defaults to the same value as `webServiceUrl`, \`http://localhost:8080`.
Can also be specified as \`pulsar://localhost:6650` for {pulsar-short} standalone instances on localhost.
a| The URL to connect to the {pulsar-short} broker or proxy
The `brokerServiceUrl` must be specified for {astra-stream}.
...
By default, filtered messages are negatively acknowledged on shared subscriptions.
If this flag is `true`, they are acknowledged in order to skip them definitively.
...
| boolean
| false
a| Offloads message filtering to the Broker nodes.
See xref:examples:pulsar-jms-server-side-filters.adoc[Server Side Filters].
...
a| Interface for custom message is `ackTimeout` policy.
You can specify `RedeliveryBackoff` for a consumer.
