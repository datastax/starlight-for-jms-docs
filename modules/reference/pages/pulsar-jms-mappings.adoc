...
{product} logs an error in case there is something wrong, but the application will continue running and is not aware of the failure.
...
When the JMS consumer receives a message that has not been produced by {product} itself and lacks the `JMSPulsarMessageType` property, it converts the message to a `BytesMessage`.
